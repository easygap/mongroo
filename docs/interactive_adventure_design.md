...
최종 갱신: 2026-08-10
상태: 전투 중심 기본 모험 + 깊은 조사 구현 기준
대상 버전: `interactive-expedition-v1.5`
...
키운 캐릭터들로 파티를 편성하고 같은 무대에서 전투·사건을 이어 가는 `기본 모험`과
노드 지도를 자유롭게 조사하는 `깊은 조사`의 게임·화면·서버·데이터·에셋 계약을 정의한다.
구현 중 세부 판단이 필요할 때는 이
문서를 우선하고, 기존 일기·성장·경제·안전 계약과 충돌하면 이 문서의 불변식을 지킨다.

> **2026-08-10 구조 개편 공지** — 기본 진행 구조와 전투·화면·발견 정보 문법은
> `docs/expedition_stage_redesign.md`(stage-battle-v2.2), 4슬롯·레벨·진화·밸런스는
> `docs/character_skill_growth_design.md`(v1.3), 정식 래스터 제작은
> `design-system/EXPEDITION_ASSET_PRODUCTION.md`가 우선한다. 이 문서의
> 노드 지도 탐험은 지역 완주 후 열리는 `깊은 조사(탐사)` 모드의 계약으로
> 유지되며, 경제·판정·안전·에셋 계약은 계속 이 문서를 따른다. 개정된 조항의
> 전체 목록은 개편 문서 2장에 있다.

## 문서 적용 전제: 감정 일기가 본편이다

이 문서가 탐험을 상세히 설명한다고 해서 제품의 중심이 탐험으로 바뀌는 것은 아니다.
몽그루의 본편은 “세상에 나쁜 감정은 없다”는 원칙 아래 마음 일기를 기록하고, 그 기록이
캐릭터의 성장과 성격으로 돌아오는 경험이다. 탐험과 전투는 그렇게 키운 캐릭터를 관상용으로
방치하지 않기 위한 선택형 부가 콘텐츠다.

제품 표면의 우선순위는 다음 순서를 지킨다.

1. 첫 화면과 가입 전 체험은 감정 기록과 성장 결과를 대표 이미지·제목·주 CTA로 보여 준다.
2. 사용자가 마음 기록과 성장 피드백을 경험한 뒤에만 탐험을 선택적으로 소개한다.
3. 탐험 안에서는 성장형이 행동·해결법·표정·대사를 바꾸되 특정 감정을 우월하게 만들지 않는다.
4. 전투는 일기 보상을 추월하거나 일기 작성을 위한 강제 숙제가 되지 않는다.
5. 마케팅 캡처와 스토어 첫 세 장에서도 일기·성장 화면이 탐험·전투보다 먼저 나온다.

회귀 기준도 명확하다. 가입 전 첫 화면에 탐험 지도를 히어로로 사용하거나, 홈의 일기 CTA보다
탐험 CTA를 더 크게 만들거나, 전투 성과가 감정 일기보다 많은 일일 XP·씨앗을 주면 출시 차단이다.

## 0. 확정 결정 요약

| 항목 | 결정 |
|---|---|
| 핵심 플레이 | 기본 모험은 `접근→조우→순차 전투→풀려남→전진`, 깊은 조사는 `노드 이동→사건→목표→귀환` |
| 조작 방식 | 기본 모험은 한 전장과 6아이콘 명령, 깊은 조사는 2D 노드 지도. 실시간 조이스틱·회피 타이밍은 요구하지 않음 |
| 한 판 길이 | 기본 스테이지 60~180초·지역 8스테이지 8~15분, 깊은 조사 8~15분. 스테이지/행동마다 자동 저장 |
...
| 스킬 구성 | 캐릭터당 고유 I·II + 감정 스킬/기록서 중 고르는 선택 I·II(`B1/B2`). 탐사 현장에는 유효 문맥 아이콘만, 전투에는 기본·4스킬·지키기 6아이콘을 노출 |
...
| 수호전 | 3인 전원의 `기본·고유 I·고유 II·선택 I·선택 II·지키기`와 순서를 매 라운드 직접 지휘. 자동 지휘는 선택 기능 |
...
| 캐릭터 에셋 | 비전투 idle은 성장 원화 재사용 가능. 전투 cast/hit/guard/recovery, 플레이어·적 공격 본체는 실제 RGBA frame sequence |
| 기본 모험 UI | 전장 72~78%, 상단 8점 진행 rail, 하단 반투명 3초상+6아이콘 dock. 지도는 선택 overlay |
| 설명 노출 | 전투 상시 설명 0자, 이벤트 28자 1줄. 스킬은 350ms long-press, 몬스터·던전·보상 전문은 도서관/기록 |
| 시청각 | 같은 지역 무대·BGM playhead를 유지하고 combat/guardian stem과 release cadence만 전환. 모든 플레이어·적 공격은 actor→travel→contact→reaction과 고유 cue를 가짐 |
| 에셋 배포 | core 21MB, 의상당 8MB, 지역 6MB + 지역 전투 3MB versioned pack. 서명·hash·원자 설치 |
...
- 조이스틱·프레임 회피를 요구하는 실시간 액션 전투, PvP, 순위표
...
| 기본 모험 | 파티 편성, 같은 무대에서 접근·전투·사건·전진 | stage 60~180초, 지역 8~15분 | 지역별 당일 첫 판 XP 6~10·씨앗 2~5·수집품 | 캐릭터 활용과 전투 중심 핵심 플레이 |
| 깊은 조사 | 노드 이동, 분기·사건·탐험 스킬, 자율 귀환 | 8~15분 능동 플레이 | 3등급 기록서 경로·이야기·지도 기록 | 공간 탐색과 복합 선택 |
| 장거리 개척 | 구간별 2인 조 편성, 2~3개 지도를 연속 탐험 | 20~35분, 구간 저장 | 직접 탐험의 당일 경제 보상 1회 + 개척 기록 | 여러 캐릭터와 함께하는 장기 탐험 |
| 자유 탐험 | 클리어한 기본 스테이지 또는 깊은 조사 재방문 | 제한 없음 | 경제 보상 없음, 이야기·지도 기록만 보존 | 다른 파티·스킬·미발견 장면 탐색 |

기존의 즉시 결과형 던전 실행은 직접 탐험 출시 뒤 구버전 호환 API로만 남긴다. 최신
앱에서 던전 카드를 누르면 바로 결과를 만들지 않고 현재 지역 무대를 연다. 최초 진입이나
파티 변경이 필요할 때만 그 무대 위 준비 drawer를 연다.

#### 2.1.1 기본 모험의 연속 화면

- 모험 허브의 full-bleed 지역 무대를 탭하면 재도전 사용자는 3초 안에 같은 장면의
  `approach`와 첫 명령으로 이어진다. 파티 변경이 없는데 편성·지도·설명 시트를 강제로
  거치게 하지 않는다.
- 접근·조우·전투·풀려남·전진은 같은 back/mid/floor/front 좌표와 BGM playhead를 쓴다.
  stage 경계는 서버 저장 단위일 뿐 흰 로딩 카드나 별도 결과 페이지가 아니다.
- 상단 8점 rail을 탭할 때만 경로 overlay를 연다. 미발견 스킬·서사·히든 보상은 잠그되,
  현재 적의 의도·대상·위력·약점·내성은 전투 공정성을 위해 즉시 보여 준다.
- 전투의 설명은 actor pose, 공격 sprite, target reaction, 환경 변화, SFX가 맡는다. 전투 중
  던전 소개·스킬 전문·몬스터 장문을 되풀이하지 않는다.
- 일반 stage 완료는 원래 물건이 돌아오고 획득 glyph가 기록장으로 이동하는 1.5~3초
  필드 연출로 끝낸다. 정확한 수량·provenance는 기록 drawer에서 확인한다.
...
3. 일반 탐험에서 다른 수확 캐릭터가 없으면 빈자리를 능력치 6의 `정원 길잡이`로 채울
   ...
   필수는 아니다.
4. 첫 기본 모험은 `기억서고 1 전투 → 2 사건 → 3 전투`의 고정 튜토리얼을 사용한다.
   깊은 조사 첫 지도는 지역 완주 뒤 숨은 길 없는 8노드 템플릿으로 따로 가르친다.
5. 기본 모험에서 대원 선택, 스킬, 약점, 지키기, 실제 접촉, 풀려남을 경험한다. 깊은
   조사에서 이동, 안전 우회, 탐험 스킬, 목표 확보, 빠른 귀환을 경험한다.
6. 기본 스테이지는 같은 무대의 풀려남·전진에서, 지역 완주와 깊은 조사는 귀환 기록에서
   보상보다 먼저 참여 캐릭터의 반응과 발견 장면을 보여 준다.
...
> 이 장의 노드·길빛·안개·숨은 간선 계약은 지역 완주 뒤의 `깊은 조사`에만 적용한다.
> 기본 모험의 진행은 `expedition_stage_redesign.md`의 8점 rail과 연속 battle trail이
> 소유하며, 아래 지도를 필수 중간 화면으로 열지 않는다.
...
수호자는 생명력 HP를 깎거나 쓰러뜨리는 대상이 아니라 지역의 규칙을 이해했는지
확인하는 3~6라운드 전술 사건이다. 사용자는 적의 다음 공격과 약점을
먼저 보고, 살아 있는 대원 전원의 기본 공격·고유 스킬·마음 지키기와
실행 순서를 매 라운드 직접 정한다. 정확한 자원·수치·연출 계약은
`docs/expedition_manual_combat.md`를 따른다.

- 대원 순서는 공유 집중력의 생성·사용 순서이자 `front` 공격을 받는 전열이다.
- 수호자는 라운드마다 `front|lowest|all` 대상과 위력을 문장·아이콘·숫자로 먼저 알린다.
- 6라운드 내 장벽을 깨지 못하거나 전원 HP가 0이면 긴급 귀환하고 미확정 보상을 잃는다.
- 피해 수치는 클라이언트가 임의로 만들지 않는다. 서버가 명령 순서대로 한 라운드를
  계산하고 `last_exchange`를 멱등 응답으로 반환한다.
- 대원 행동은 각 760ms, 적 행동은 940ms로 분리해 피해 주체와 순서를 보이고,
  전체 라운드를 한 번의 결과 영상으로 뻐기지 않는다.
- 화면 흔들림은 반격 충돌 한 번, 3.5px 이하이며 섬광은 한 번만 쓴다. 피해 숫자,
  장벽 막대, `방어|진정` 문구를 함께 사용해 색이나 파티클만으로 결과를 전달하지 않는다.
...
이 여섯 family는 전투에서도 같은 정체성을 유지한다. stage 3의 안정 주결은
`emotion.primary`, stage 4에서 공개 가능한 보조결은 `emotion.secondary`로 열리며,
사용자는 선택 스킬 I·II에 감정 스킬 또는 기록서를 자유롭게 교체한다. 전투 payload와
20원소→여섯 성장결 약점·내성 수치는 `expedition_combat_core_design.md` 3~4장과
`combat_identity_progression_design.md`가 단일 원본이다.
분석이 부족하면 모아결 스킬을 동급 임시 선택지로 제공하고, 특정 스킬을 얻기 위해
특정 감정의 일기를 쓰라고 권하거나 진행 압박을 만들지 않는다.
...
초반 seed, 중간 echo, 목표/선택 발견 payoff를 하나의 이야기로 연결한다. thread
12종의 실제 원고, 세계관, 수호자·관계 서사는 `docs/expedition_story_design.md`가
단일 원본이다.
...
> **2026-09-01 구현:** 세 방향을 `app/content/expeditions/journey.py`에 넣고
> `/adventure/journeys` 아래에 열었다. 표의 `자체 캐릭터 최대 사용`은 따로 값을
> 두지 않는다 — 구간마다 두 자리이므로 언제나 `구간 수 × 2`이고, 두 값을 각각
> 들고 있으면 구간 수만 고치는 날이 온다.
>
> **구간은 새 게임 모드가 아니다.** `expeditions.start_run`이 만드는 평범한
> 탐험 run에 `journey_id`만 매달았다. 그래서 걷기·사건·전투·귀환이 전부 지금까지의
> 코드 그대로이고, 앱도 구간을 걸을 때는 기존 탐험 화면으로 보낸다.
>
> 표를 셋으로 나눈 설계(`journey_legs`·`journey_members`·`journey_actions`)는
> **한 표로 합쳤다.** 구간은 최대 셋, 참가자는 최대 여섯이고 언제나 통째로만
> 읽는다. `UNIQUE(journey_id, plant_id)`가 하던 일은 서비스가 지키고,
> `journey_actions`가 하던 멱등은 이미 있는 `Idempotency-Key` 경로가 대신한다.
> 합동 수호전이 판 상태를 run 한 줄에 얹은 것과 같은 판단이다.
>
> **2026-09-01 가치 예산:** 9.1의 예산·칸을 네 지역 팩에 싣고 일반 탐험과
> 장거리 개척이 **같은 함수**(`content/expeditions/loot_budget.py`)를 지나게
> 했다. 한쪽에만 넣으면 두 경로의 loot 규칙이 갈리므로 둘을 함께 고쳤다.
>
> 예산이 실제로 고를 거리를 만들려면 후보가 예산보다 많아야 한다. 지금까지
> run 하나가 남기는 후보는 목표 재료 **하나뿐**이라 어느 지역이든 예산이
> 남아돌았다. 그래서 사건을 매듭지은 자리마다 현장 재료 하나를 후보로
> 남긴다 — 우회로 끝나면 남지 않으므로 판정이 후보 수를 바꾼다.
>
> 관측실만 목표 재료가 가치 2다. `가치 2 목표 + 가치 1 재료 2개`로 예산 4가
> 정확히 찬다(9.1의 예시 그대로).
>
> 자동 채우기는 **목표 재료 우선, 그다음 먼저 발견한 순서**다. 예산이 1 남았는데
> 다음 후보가 2면 건너뛰고 그 뒤의 1짜리를 담는다. 사용자가 고른 조합이 예산이나
> 칸을 넘으면 조용히 잘라 내지 않고 422로 돌려준다 — 잘라 내면 무엇이 빠졌는지
> 알 길이 없다.
>
> 화면은 자동 채우기를 **흉내 내지 않는다.** 아무것도 고르지 않으면 빈 목록을
> 보내고 서버가 채운다. 같은 규칙을 두 곳에 두면 언젠가 갈라지고, 그때 화면이
> 약속한 것과 실제로 담아 온 것이 달라진다.
>
> **XP·씨앗도 이 표대로 맞췄다.** 콘텐츠 팩이 `6·7·8·9 / 2·2·3·3`으로 실려
> 있어서 우물정원부터 관측실까지 세 지역이 표보다 낮았다. 어느 검사도 그 차이를
> 보지 않아 조용히 어긋난 채였다. 이제 `test_expedition_reward_bands.py`가 네
> 지역의 XP·씨앗·예산·칸을 이 표와 직접 맞대고, 위 문단이 약속한 두 가지도
> 함께 센다 — **일기 한 편이 가장 먼 탐험과 순찰을 합친 것보다 크다**는 순서와,
> **일기 40 + 관측실 10 = 50**이 하루 상한과 정확히 같다는 것.
>
> **`오늘의 성장 효율` 표에 `직접 탐험` 줄을 세웠다.** 그 자리에는 `던전`
> 한 줄뿐이었는데, 그건 아직 살아 있는 구버전 던전(`/adventure/dungeons/…`)의
> 값이다. 직접 탐험을 권하는 바로 그 화면에서 다른 활동의 숫자를 읽게 되어
> 있었다. 이제 두 줄이 서로 다른 이름(`직접 탐험` · `발견한 던전`)을 쓰고,
> `발견한 던전`은 같은 화면 아래쪽 구역 제목과 같은 말이라 무엇을 가리키는지
> 화면 안에서 이어진다.
>
> 직접 탐험 줄은 지역마다 값이 달라 한 값으로 못 적으므로 `6~10 XP · 씨앗 2~5`
> 처럼 폭으로 읽어 주고, 그 폭은 **콘텐츠 팩에서 직접 잰다.** 손으로 적어 두면
> 지역 보상을 올릴 때 화면만 옛 숫자를 말한다 — 이 표가 그렇게 어긋났었다.
...
- 중앙 주 무대: 현재 노드의 장소 원화, 심도, 위험도, 장소 설명. 원화 안에
  필수 텍스트를 굽지 않고 코드 HUD와 아래 설명에 동일 정보를 놓는다.
- 중앙 보조: 확대·이동 전체 지도와 현재 파티 토큰. 지도는 현재 장소를 대체하지
  않고 경로·방문·되돌아갈 수 있는 곳을 파악하는 정보 레이어다.
- 다음 방 선택: 원형 아이콘만 나열하지 않고 장소 원화 미리보기, 방 이름, 장소 종류,
  심도, 위험도, 길빛 비용을 한 카드에 보여 준다.
...
> 이 절은 `깊은 조사`, 장거리 개척, 지역 8·수호전의 귀환에 적용한다. 기본 모험의 일반
> 스테이지는 별도 결과 화면을 열지 않고 같은 무대의 `release→advance`와 기록 drawer로
> 끝낸다.
  ...
  - code: archive-loop-a
    nodes:
      - code: entrance
        ...
        scene_label: 폐허 던전
        scene_description: 버려진 온실 아래의 석문이 열렸어요.
      ...
      - code: west-01
        ...
        scene_label: 침수 동굴
        scene_description: 수정빛 물길 너머 두 갈래 통로가 보여요.
...
- 위험은 유리 파편, 깊은 물, 추락처럼 현실적 공포를 과장하지 않고 식물 세계의
  막힌 덩굴·빛·소리·장치로 표현한다.
- 기본 모험의 필드 사건은 본문 28자 1줄, 즉시 결과 18자를 상한으로 둔다. 깊은 조사의
  선택 sheet와 도서관 원고는 사건 본문 90자, 선택 label 24자, 결과 140자를 권장 상한으로 둔다.
...
기억서고 통합 지형은 `expedition-moss-archive-terrain-v3.webp`를 쓴다. 석문 입구,
침수 동굴, 뿌리 땅굴, 메아리 우물, 반매립 보물고, 장부지기 소굴, 등반 탑, 귀환
터널을 한 지형에 직접 그려 길·수로·뿌리 다리로 연결한다. 랜드마크를 원형 아이콘이나
고립된 카드처럼 그리지 않는다. 런타임은 실제 1~3명 `PlantView`를 현재 좌표에 얹고
940ms 동안 원화의 길 중심선 polyline을 따라 이동시킨다. 발자국도 같은 polyline을
사용한다. 지도 템플릿은 간선만 바꾸며 랜드마크 좌표를 override하지 않는다. 배경에
pan·scale을 적용하면 좌표가 어긋나므로 금지한다. 장소별 원화는 사건 또는 스킬
연출이 시작될 때만 현장 확대 무대로 사용한다.

`dungeon_gate|flooded_cave|root_tunnel|echo_well|treasure_vault|
monster_den|moon_tower` 일곱 `scene_key`는 현장 확대에 쓴다. `moon_tower`는 나선
계단과 3개 이상의 층을 한 구도에 보여 주고, 후속 등반 모드는 `depth_label`과 층별
node만 늘려 같은 장면 계약을 재사용한다.
...
| 전투 layer | 지역당 combat stem 1개, 같은 BGM playhead 위 180~350ms 교차 |
| 수호자 layer | 지역당 20~30초 타악/벨 stem 1개, combat 위 한 층, 별도 곡 전환 없음 |
| 풀려남 | 지역당 두 음 release cadence 1개, stem을 걷은 뒤 한 번만 재생 |
| 환경음 | 지역당 2개, -28~-24 LUFS, 반복이 눈에 띄지 않게 15초 이상 |
| 이동 | 잎·화분 재질 4종, 120ms 이하 |
| 발견 | 일반/이야기/목표 3단계, 500ms 이하 |
| 스킬 | 고유 I·II 20종의 짧은 signature, 700ms 이하 |
| 적 공격 | 일반/큰 엉킴 16공격 + 수호짐승 8공격의 고유 release signature |
| 접촉 | leaf/paper/water/wood/stone/guard 6종, contact frame ±1 frame |
| UI | 방 공개, 선택 확정, 자원 회복, 저장 완료 |

- 트리 90노드마다 별도 효과음을 만들지 않는다. 고유 스킬 signature 20종 위에 길 읽기·상황
...
- 노드 상태를 색만으로 구분하지 않고 비콘 모양·발자국·문구를 함께 사용한다.
...
- 서버가 허용한 일곱 `scene_key`와 Flutter 에셋 매핑의 1:1 일치, 번들 파일 누락 0
- 320px/200% 글자에서 파티 준비, 사건 선택, 결과 화면 overflow 0
- 320px/200% 글자에서 현재 장소·심도·위험도·다음 장면 미리보기 overflow 0
...
31. 던전 입구 → 침수 동굴/뿌리 땅굴 → 보물고 → 몸스터 소굴 → 기억탑 → 귀환
    땅굴을 실제 원화로 확인하고, 장면 전환 중 단색·이전 장면 재등장·누락 프레임 0
32. 가입 없는 체험에서 두 갈래의 실제 장면 카드를 각각 선택 → 선택한 원화 전환 →
    보물고 귀환 장면 확인
...
- stage 2~4 비전투 transform, 전투 cast/hit/guard 실제 pose와 stage 5 10종×6형×7 action base strip
- garden-daily·city-night의 같은 frame index outfit strip
- 플레이어·적 전투 공격 family 62세트, 플레이어 VFX 712F·actor pose 200F,
  엉킴 12종 5동작·적 공격 24종, UI SVG 83·코드 icon 22, 네 지역 raster 64개
- preload·동기 프레임·reduce motion
- 지역 BGM·combat/guardian stem·release cadence·환경음·플레이어/적 공격 SFX 102개
...
- **M1 완료된 추가 범위:** 6단계 코치 마크와 현재 도움말 재생, 10종 고유
  ...
  원래/지역 보정 능력치 스냅샷, 보상보다 먼저 보여 주는 파티 귀환 기록, 가입 없는
  직접 탐험 체험, 기억서고 일곱 장소 원화·장면 메타데이터·미리 적재·교차 전환을 구현했다.
...
- [ ] 탐사 현장은 유효한 문맥 아이콘만, 전투는 기본·고유 I·II·선택 I·II·지키기
  6아이콘이며 트리 노드는 기존 스킬의 적용 배지로 나타난다.
...
- [ ] stage 2~4 비전투 외형과 전투 pose, stage 5 base 420·두 의상 840 runtime strip,
  전투 공격 family 62세트·플레이어 VFX 712F·actor pose 200F·엉킴 12종·적 공격 24종,
  지역 raster 64, UI SVG 83·코드 icon 22, 오디오가 manifest와 일치한다.
...
- 유기적 공격 본체·몬스터 몸체를 코드 path/원/선/gradient로 정식 제작하는 처리
- 플레이어 공격만 전용 스프라이트로 만들고 적 공격은 공용 파동으로 돌려 쓰는 처리
...
## 부록 E. 2026-08-06 지형 탐험·전투 가독성 재검증

2026-08-06 기준 공식 개발사·플랫폼 자료를 다시 조사했다. 몽그루에 그대로 복제한 것이
아니라, 모바일 선택형 탐험에 맞는 공통 원칙만 추려 다음처럼 반영한다.

| 공식 사례 | 확인한 최근 방향 | 몽그루 구현 판단 |
|---|---|---|
| ELDEN RING NIGHTREIGN | 변화하는 한 지도 안에서 적·전리품·보스 위치를 보고 경로를 고르며 영웅 고유 스킬과 팀 조합을 사용 | 목적지를 아이콘 목록으로 분리하지 않고, 모든 랜드마크와 갈림길을 하나의 통합 지형에 배치한다. 파티 스프라이트가 선택한 경로를 실제 이동한다. |
| ECHOES OF AINCRAD | 미지의 필드·던전을 직접 매핑하고 보물·기믹을 발견하며, 바위·덩굴·틈을 도구로 통과. 적 조우도 필드에서 이어짐 | 발견 전 메타데이터를 감추고 이동으로 지도를 연다. 동굴·땅굴·탑은 원형 버튼이 아니라 물길·뿌리 다리·계단이 붙은 지형지물이어야 한다. |
| Beast of Reincarnation | 환경이 실시간으로 변하고 동료 기술 선택 중 시간을 늦춰 액션과 전술 명령을 결합 | 조이스틱 액션을 억지로 축소하지 않고, 현장 장면은 계속 움직이되 캐릭터·스킬·경로 선택은 읽을 시간을 주는 전술 명령 방식으로 유지한다. |
| The Adventures of Elliot | 숨은 동굴·폐허와 동료 탐색 능력, 전투와 탐험이 끊기지 않는 구성 | 사건 패널로 전환하더라도 통합 지형과 현재 현장 맥락을 유지하고, 선택 캐릭터를 현장 위에 직접 렌더한다. |
| Diablo IV 2026-07-28 패치 | VFX 크기·피해 표시·몬스터 생성·던전 전환 문제를 지속적으로 수정 | 화려함보다 판정 일치와 가독성을 출시 기준으로 둔다. 서버 피해 수치, 막대, 숫자, 공격 예고, 피격/방어 문구가 한 프레임 계약을 공유한다. |
| Hades II | 빠르고 반응성 있는 행동과 변화하는 장소·도전을 반복 run의 핵심으로 사용 | 반복 보상 인플레이션 대신 경로·사건·캐릭터 행동자·발견 기록 조합을 바꾼다. |

이 조사에서 도출한 결론은 다음과 같다.

1. ‘탐험’은 배경 위에 목적지 아이콘을 얹는 것이 아니라, 목적지 자체가 지형과 이동
   기믹으로 읽혀야 한다.
2. 스킬 연출은 파티클 수가 아니라 `준비-궤적-충돌-반격 예고-대응-수치 결과`의 원인과
   결과가 보이는지가 품질 기준이다.
3. 몽그루의 핵심 사용자는 반사신경 전투를 요구받지 않는다. 대신 선택 뒤 실제 캐릭터와
   수호자가 같은 현장에서 행동하고, 서버 판정 수치가 눈에 보이는 전술 교환을 제공한다.
4. 수호자는 죽이는 몬스터가 아니지만 공격하지 않는 장식도 아니다. 수호 장벽은 피해를
   받고 수호자는 예고 후 반격하며, 캐릭터는 방어하거나 결의 피해를 받는다.

근거 자료:

- [Bandai Namco — ELDEN RING NIGHTREIGN Gameplay Guide](https://www.bandainamcoent.com/games/nightreign/gameplay-guide)
- [Bandai Namco — ECHOES OF AINCRAD: A Guide to Aincrad](https://www.bandainamcoent.com/news/echoes-of-aincrad-a-guide-to-aincrad)
- [PlayStation Blog Japan — ECHOES OF AINCRAD launch guide](https://blog.ja.playstation.com/2026/07/08/20260708-eoa-launch-reminder-s/)
- [PlayStation Blog — Beast of Reincarnation launches August 4, 2026](https://blog.playstation.com/2026/02/12/beast-of-reincarnation-launches-on-ps5-august-4-2026/)
- [Xbox Wire — The Adventures of Elliot: Exploration, Combat, Discovery](https://news.xbox.com/en-us/2026/06/18/the-adventures-of-elliot-exploration-combat-discovery/)
- [Blizzard — Diablo IV Patch Notes, July 28, 2026](https://news.blizzard.com/en-us/article/24287406/diablo-iv-patch-notes)
- [Supergiant Games — Hades II official updates](https://www.supergiantgames.com/blog/)

## 부록 F. 2026-08-10 시장·최신 RPG 재검증

부록 C는 장기 성장과 여러 캐릭터 활용, 부록 E는 지형·전투 가독성을 검증했다. 이번
재검증은 2026년 시장 환경과 최신 모바일·파티 RPG가 **첫 전투, 캐릭터 존재감, 감각
피드백, 출시 후 운영**을 어떻게 구성하는지 확인했다. 전체 수치·사례·출처와 채택/배제
판단은 `adventure_game_trend_review_2026.md`가 단일 원본이다.

### F.1 확인한 변화

1. 모바일 다운로드는 줄어도 플레이 시간은 유지되는 성숙 시장이므로 설치 수보다 첫
   세션 약속과 반복 유지가 중요하다.
2. 최신 파티 RPG는 3~4인을 단순 초상화가 아니라 전장 행동, tag, assist, 음성으로
   계속 느끼게 한다.
3. 캐릭터 고유 공격뿐 아니라 적 피격 반응, 예고, 타기팅 설정, 캐릭터별 연습장이
   전투 품질의 일부가 됐다.
4. 액션과 명령을 결합한 게임은 명령 중 시간을 늦춰 생각할 틈을 주며, 플랫폼은 터치·
   오디오·햅틱·모션 조절을 하나의 상호작용 품질로 본다.
5. live ops는 계속 중요하지만 이벤트 수를 많이 겹치는 방식은 몽그루의 정서 안전·
   제작 규모·비FOMO와 맞지 않는다.
...
| 영역 | 확정 결정 | 직접 원본 |
|---|---|---|
| 첫 전투 | 재도전 명령 p95 3초, 신규 첫 contact p95 15초 | `expedition_stage_redesign.md` |
| 화면 | 통합 전장 72~78%, 3인 전장 존재, 6아이콘 독 | `expedition_stage_redesign.md` |
| 전술 상세 | 350ms hold의 전술 집중, 상세 높이 40% 이하 | `expedition_stage_redesign.md` |
| 도움 | 12초 무입력 때 합법 행동 하나만 맥동, 자동 실행 0 | `expedition_stage_redesign.md` |
| 캐릭터 체험 | 전 캐릭터 20~30초 무보상 연습장 | `character_skill_growth_design.md` |
| 공격 품질 | player/enemy 모두 release·travel·contact·reaction·고유 SFX | `EXPEDITION_ASSET_PRODUCTION.md` |
| 감정 | 동등한 여섯 상성, 스트레스·붕괴·벌점 변환 금지 | `character_skill_growth_design.md` |
| 운영 | 세로 슬라이스 합격 뒤 8~12주 영구 pack | `content_strategy.md` |
| 홍보 | 실제 빌드의 15초 전투 clip, 콘셉트의 gameplay 오인 금지 | `content_strategy.md` |

### F.3 유행을 따라 넣지 않는 것

- 실시간 PvP, 길드 의무, cross-play는 초기 범위에 넣지 않는다.
- 배틀패스·7일 출석·기간 한정 전력·다중 이벤트 통화·빨간 점을 만들지 않는다.
- 몬스터 포획·합성은 몽그루의 엉킴 풀기와 도감 역할을 흐리므로 복제하지 않는다.
- 감정 기록을 전투 스트레스, 붕괴, 부상, 실패율, 보상 감소로 바꾸지 않는다.
- 자동 파견은 기존 자동 순찰 이상으로 키우지 않고 직접 모험의 성장 효율을 넘기지 않는다.
- PC·협동은 계정 snapshot과 responsive UI가 확장 가능하도록만 설계하고 모바일 세로
  슬라이스 합격 뒤 별도 승인한다.

### F.4 구현 순서

1. 기억서고 한 전장, 뽀또 덩굴 1종, 장부 엉킴 공격·피격·풀려남, 실제 SFX·햅틱을
   출시 품질로 완성한다.
2. 사용자에게 첫 contact 시간, 공격 인과 이해, 5분 뒤 자유 회상, 소리 없는 완주를
   검증한다.
3. 같은 제작 파이프라인으로 3인 파티와 적 3종·수호자까지 넓힌다.
4. 이 세로 슬라이스가 합격한 뒤에만 나머지 지역, 캐릭터 family, 영구 pack을 제작한다.

이 순서를 어기고 지역·이벤트·상점부터 늘리는 것은 2026 시장 조사 결과와 이 문서의
제품 전략을 모두 위반한다.
