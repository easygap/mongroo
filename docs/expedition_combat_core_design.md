...
> 이 문서는 전투 코어의 누적 설계 기록이다. 캐릭터 15종, 고유기 30종,
> 감정 융합 180종과 43,200전 최신 검증값은
> [탐험 전투·캐릭터 성장 시스템 v8](combat_system_design_v8.md)을 기준으로 본다.
...
> 2026-08-11 `combat-kit-v7` 로스터 확장으로 바뀐 캐릭터 수, 스킬 수, 해금 레벨,
> 희귀도 곡선, 역할 스탯, 프리미엄 지원가, 3페이즈 보스 계약은
> `combat_roster_expansion_2026.md`가 우선한다. 아래 v5/v6 진단 수치는 개정 근거로
> 보존한 역사 기록이며 현재 로스터 완료율로 해석하지 않는다.

### 2026-08-11 v7 로스터·보스 구현 스냅샷

- 플레이 가능 캐릭터는 12종, 고유기는 24종이며 각기 다른 code·motion/VFX family를
  가진다. 고유 I은 Lv3, 고유 II는 Lv7에 분리 해금한다.
- 공격·생존·지원·제어 전투 스탯을 캐릭터 base, 레벨 성장률, 희귀도, 감정 보너스로
  계산한다. 희귀도 5는 Lv1부터 근소하게 강하고 Lv30 상한도 가장 높다.
- 백화는 회복·보호·T3 1인 소생, 세렌은 같은 라운드 후속 증폭·취약·적 위력 감소를
  서버 권위로 판정한다.
- `battle-state-v3` 보스는 장벽 66%·33%에서 약점·내성·의도 cycle·위력·집중 보상과
  연출 tier를 바꾼다.
- 앱은 기믹 요약, 역할 스탯, 보스 페이즈, tier별 SFX·카메라, 감정색 두 층을 읽는다.
  전용 sprite가 없는 family의 production fallback 상태는 그대로 공개한다.
- `balance version 2`는 12종×6감정×5등급×권장 레벨·지역·스테이지 조합 11,520셀을
  정책 3종, 총 34,560전투로 재생했다. 기본 10종 격차, 스테이지 템포, 상성 기여,
  슬롯 사용, 세렌 최단 공략, 백화 최고 생존을 포함한 8개 게이트가 모두 통과했다.
