...
| 효과음 초기화 | 수령 시 재생기 생성과 음원 준비 | 화면을 읽는 동안 두 효과음을 준비하고 재생기를 재사용 | 보상 순간의 준비 비용과 재할당을 줄이도록 |
...
- 효과음 끄기를 따르고, 시스템의 모션 줄이기나 게임의 연출 줄이기에서는 이동 없이 결과를 바로 표시한다. 접근성 안내는 증가 중간 값 대신 확정된 보상과 잔액을 읽는다.

비행체는 지급량과 관계없이 최대 8개다. 파티클은 `CustomPainter(repaint: animation)`로 해당 영역만 다시 그린다. 별과 씨앗의 `Path`, 그리기에 쓰는 `Paint`도 재사용한다. 캐릭터와 문구는 애니메이션의 고정 자식으로 두고, 별도의 `RepaintBoundary`로 분리했다. 보상·해금 연출이 끝나면 반복 타이머나 계속 도는 애니메이션은 남기지 않는다.
...
추가로 일기 저장 → 성장 확인 → 퀘스트 완료를 연속 실행했다. 보상 소리는 두 번째에도 재생됐고, 그 사이 음원 요청은 5건에서 늘어나지 않았다. 일반 CPU 설정에서 퀘스트 수령 구간 2초의 121개 프레임은 95백분위 16.8ms, 최대 16.9ms였다.

CPU를 4배 느리게 제한하고 화면이 안정된 뒤 수령한 2.5초 스트레스 구간에서는, 도형·음원 재사용 전후의 95백분위가 83.5ms와 50.0ms였다. 다만 첫 창 전환의 최대 간격은 250.2ms와 266.9ms로 여전히 컸다. 이는 각각 한 번의 참고 측정이며 저사양에서 끊김이 사라졌다고 판단할 근거가 아니다. 실제 목표 기기에서 첫 전환과 장시간 플레이를 더 확인해야 한다. 게임의 연출 줄이기를 켜면 보상·해금도 움직임 없이 바로 표시한다.


- Flutter 분석: 오류·경고 없음.
- Flutter 전체 테스트: 564개 통과.
- 보상·일기 성장·퀘스트 관련 서버 검사: 23개 통과.
- 새 검사: 씨앗 도착과 잔액 반영, 보상 없는 응답, 더 최근 잔액, 닫기·백그라운드 전환, 시스템·게임의 모션 줄이기, 320px·글자 200%, 해금 중 닫기, 고정 자식의 프레임별 재생성 방지, 도감 80종의 지연 생성, 다른 화면에서 받은 재화의 상점 반영.
