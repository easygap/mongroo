...
## 현재 README의 캡처 — `release-2026-09-22/`

2026년 9월 22일의 Flutter Web 릴리스 빌드와 테스트용 API를 연결해 촬영했다.
기본 캡처는 390×844 세로 화면이다. 성장 도감은 여섯 타입을 한 번에 볼 수 있게
540×1320에서 촬영했다. 개인정보가 없는 별도 계정을 사용했으며, 화면을 합성하거나
실제 게임에 없는 요소를 덧붙이지 않았다.

| 파일 | 화면 |
| --- | --- |
| `home.png` | 캐릭터와 성장 상태 |
| `diary.png` | 일기 작성 |
| `explore.png` | 지역 지도와 스테이지 선택 |
| `battle.png` | 적의 공격 예고와 행동 선택 |
| `settings.png` | 전투 속도·효과·음악·효과음 설정 |
| `garden.png` | 방 꾸미기 |
| `growth.png` | 감정별 성장 도감 |

다음의 이전 화면들은 변경 이력을 위한 보관 자료이며 현재 README에는 사용하지 않는다.

## 이전 README의 캡처 — `web/readme-current/`
