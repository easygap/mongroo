...
10~13번은 2026-08-24 출시 후보 웹 빌드(release)를 1280×860 데스크톱 뷰포트에서
실행해 다시 촬영했다. 칸 단위 걷기 개편 이후의 도트 지형·오토타일 벽·조형물이
모두 실제 렌더링이며, QA 전용 계정으로 각 지역 1스테이지를 직접 걸은 뒤 지도
카드 영역만 잘라 냈다. 14~16번은 2026-08-14 빌드 기록으로, 아날로그 걷기와
발자국 등 지금은 없는 조작이 담겨 있어 현재 기능 설명에는 쓰지 않는다.

`web/readme-current/`는 README가 실제로 링크하는 최신 캡처만 모아 둔다.
2026-08-24 출시 후보 웹 빌드에서 같은 QA 세션으로 촬영했다.

```text
home-current.webp                  홈 화면 (2026-08-19 촬영, 화면 변경 없음)
expedition-tile-world-walk.gif     방향키로 걷는 장면을 실시간 화면 녹화에서
                                   10fps로 다시 뽑은 것. 프레임 합성 없음
expedition-tile-world-current.webp 석주·등불·서가·기록함이 놓인 방 (1280 와이드 크롭)
expedition-shard-speech.webp       떨어진 이름표를 주웠을 때의 말풍선 (390×844 모바일)
```
