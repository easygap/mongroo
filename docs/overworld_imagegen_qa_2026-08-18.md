...
## 2026-09-01 후속 — 체크무늬 배경을 되살려 쓴 방법

위의 `실행 방식`은 런타임 채택 0개로 끝났고, 이유 하나가 **체크무늬로 구워진
가짜 투명 배경**이었다. 그때는 "체크무늬 제거 후처리로 가장자리를 훼손"한다고
보고 전부 뺐다. 품종별 걷기 도트 열일곱 벌을 만들면서 같은 벽을 다시 만났고, 이번에는
넘었다.

핵심은 **색으로 지우지 않고 가장자리에서 번지며 지우는** 것이다. 체크무늬는 두
가지 밝은 회색이고, 캐릭터에도 밝은 색(얼굴·흰 가운)이 있다. 전역 색 키로
지우면 그 밝은 색까지 같이 날아간다. 대신 판의 네 변에서 물을 채우듯 번져
들어가면, 어두운 외곽선으로 둘러싸인 안쪽 밝은 색은 손대지 않는다. 실제로 넉
장 × 열일곱 품종, 예순여덟 장 모두 가장자리 훼손 없이 알파가 열렸다.

두 번째 벽은 **줄이는 방법**이었다. 1536픽셀 판을 96픽셀 칸으로 곧장 줄이면
자수·머리카락 같은 잔무늬가 픽셀마다 튀는 값으로 남는다. 위의 `AI-slop 방지
자동 관문`이 재는 그 값이다 — 마법사 시트가 자글거림 0.238로 걸렸다(한계
0.16). 알파를 곱한 뒤 세 번에 나눠 줄이니 0.146이 됐다. 관문을 느슨하게 하지
않고 그림을 고쳤다는 점이 중요하다.

절차는 `design-system/scripts/prepare_walker_strip.py`에 있고, 결과와 프롬프트는
`design-system/concepts/expedition-walker-species-v1/README.md`에 있다. 바닥
타일에 같은 방법을 다시 시도할지는 별개 판단이다 — 바닥은 **사방 이음매**를
증명해야 하고, 그건 이번에 푼 문제가 아니다.
