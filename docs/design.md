...
3. **식물 키우기**
   ...
   - 상점·도감의 주 캐릭터 10종과 별도로, 일기 식물은 5개 성장 단계와 6개 감정 형태를 가짐
4. **식물 대화**
...
| 테이블 | 목적 | 핵심 제약 |
|--------|------|-----------|
| `ai_jobs` | 감정 분석, 대화 생성, 리포트 요약의 영속 큐 | `UNIQUE(job_type, resource_type, resource_id, input_version)` |
| `idempotency_keys` | 재시도된 쓰기 요청의 최초 응답 재생 | `UNIQUE(user_id, route_scope, idempotency_key)` |

`ai_jobs`는 `status=pending|running|succeeded|failed`, `attempts`, `available_at`,
`locked_at`, `last_error_code`를 갖는다. 프로세스 시작 시 일정 시간 이상 `running`인 job을 `pending`으로
되돌린다.
...
- `plant_species.asset_manifest.growth`는 품종별 씨앗 모양, 화분/배양관, 희귀도 효과와
  자산 namespace를 담는다. `PlantView`는 이 정체성과 5단계·감정 레이어를 조합한다.
...
- 식물 표현형은 감정을 평가하는 점수가 아니다. `sunny`, `rainy`, `ember`, `moonlit`,
  ...
  사용하지 않는다.
- `farm_layouts.layout`은 보유한 `user_item_id`와 x/y/rotation/z-index만 저장한다. rotation은
...
| Method | Path | 상태 | 설명 |
|--------|------|------|------|
| POST | `/auth/signup` | 201 | 가입, 기본 활성 식물과 무료 아기 화분 캐릭터 생성 |
...
성장 곡선은 첫 기록에서 즉시 새싹을 보여 주고, 기록 중심 이용자도 대략 3주 안에 첫 식물을 만개시키는 것을 기준으로 한다.
...
- 상점/도감/마이팜: 잠긴 방 테마도 실제 배경으로 미리 보고 획득 조건·진행도를
  확인한다. 획득 직후 `바로 적용` 또는 `나중에`를 선택하며, 적용은 저장 가능한
  마이팜 draft로 연결한다.
- 안전 화면: 112/119, 109, 1577-0199 원터치 연결과 앱 이탈 경로를 명확히 제공

### 8.4 캐릭터 자산

- 성장 식물은 현재 `PlantView`의 `CustomPaint`로 5단계를 그리며, 최근 기분은
  눈·입 표정에만 반영한다. 보상·성장 계산에는 쓰지 않는다.
- 상점/도감/마이팜은 서버 `asset_manifest.asset_key`를 앱 번들 WebP에 매핑한다.
  캐릭터는 공용 `GardenItemVisual`과 `motion_key`를 사용해 화면마다 같은 인상과 모션을 유지한다.
- 캐릭터·친구·데코·품종 cutout은 투명 배경, 방 테마는 고정 가로 비율을 사용한다.
