...
- 여섯 감정 성장형은 같은 시점·같은 예산의 전용 탐험/전투 스킬을 배운다. 몬스터의 고정 약점·내성은
  여섯 결에 균등하게 분산하고 확정 전에 공개하며, 상성은 전투 방법만 바꾼다. 특정 감정 기록을
  유도하거나 성장 속도·입장권·희귀도·경제 보상에 유불리를 만들지 않는다.
- 기본 모험은 `접근→조우→전투→풀려남→전진`이 한 지역 무대에서 이어지는 전투 중심 화면을 쓴다.
  전투 중 설명문은 상시 노출하지 않고 스킬은 long-press, 서사·생태·히든 보상은 도서관/기록에서
  읽는다. 플레이어와 적의 actor·공격·접촉·피격은 모두 정식 스프라이트와 고유 음향을 사용하며,
  코드 파동·색상 복제·불투명 카드 패널을 출시 아트로 인정하지 않는다.
