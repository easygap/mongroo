...
- 가입 여부를 결정하기 전에 핵심 재미를 판단할 수 있어야 한다. 가입 전 체험은 서버 계정을
  만들지 않고 기기 저장소만 사용하며, 마음 기록 → 성장 → 직접 탐험의 핵심 루프를 짧게 완주한다.
...
- 이끼 기억서고의 인터랙티브 지도는
  `app/assets/adventure/expedition-moss-archive-map.webp` 한 장을 배경으로 쓰고,
  경로·노드·비용·현재 위치·선택 상태는 이미지에 굽지 않고 Flutter가 그린다. 배경은
  작은 화면에서도 갈림길이 읽히도록 좌우 길띠와 중앙 랜드마크를 남기며, 저주파
  시차·안개·광점만 분리해 깊이를 만든다. 실시간 3D 엔진은 설치 크기와 발열 대비
  핵심 선택 경험을 개선하지 않으므로 첫 지역에는 도입하지 않는다.
