...
| 보스 | 3페이즈 | 100%·66%·33%에서 상성·패턴·위력·연출 전환 |
| 밸런스 검증 | 43,200전 | 8개 출시 게이트 전부 통과 |
...
- [GDC — Boss Battle Design Fundamentals](https://www.gdcvault.com/play/1025398/Boss-Up-Boss-Battle-Design)
- [GDC 2026 — State of Level Design](https://schedule.gdconf.com/session/state-of-level-design-2026/917869)
...
피해 직후 경계를 넘으면 서버가 `boss_phase` 이벤트를 보내고, 앱은 전용 봉인 파쇄
스프라이트와 SFX를 먼저 재생한 뒤 새 패턴을 표시한다. 큰 피해로 두 경계를 동시에
넘으면 두 전환을 순서대로 기록한다. 장벽이 0이면 불필요한 페이즈 연출 없이 승리를
우선한다.
...
재화 가격은 사용자의 외형 선호를 벌주는 수단이 아니다. 가격이 높을수록 평균
능력치를 무조건 덮어쓰는 대신 역할 압축과 숙련 상한을 높인다. 실제 전수 검증에서
프리미엄 생존 승률 하한은 일반 캐릭터 평균 이상이며, 무료·일반 캐릭터 간 승률
격차는 5%p 이하다.

## 11. 밸런스 결과와 출시 게이트

결정론적 엔진으로 15종×6감정×5희귀도×4지역×권장 레벨 2개×스테이지 형태
4개를 세 정책으로 계산했다. 총 43,200전이며 결과 원본은
`docs/expedition_combat_balance_report.json`이다.

| 게이트 | 결과 |
|---|---:|
| 튜토리얼 | 승률 100%, P90 1라운드 |
| 일반 | 승률 100%, P50 3라운드, 1라운드 클리어 0 |
| 엘리트 | 승률 99.92%, P50 2라운드, 1라운드 클리어 0 |
| 혼합 숙련 | 생존 정책 승률 85.00%, P50 3라운드 |
| 상성 실효값 | 약점 행동 피해 +43.30% |
| 상성 선택 | 약점 적중률 +15.56%p |
| 일반 캐릭터 편차 | 최대 5.00%p |
| 프리미엄 역할 | 세렌 최단 공략, 백화 최고 생존 잔여 HP, 하한 통과 |

8개 게이트는 모두 통과했다. 이 수치는 새 캐릭터나 계수를 바꿀 때 리포트를 다시
생성해야 하며, checked-in 리포트가 하나라도 실패하면 테스트가 배포를 막는다.

## 12. 완료 기준과 남은 실기 QA

코드·에셋 제작 게이트는 완료됐다. 다음 항목은 기능 미구현이 아니라 출시 기기별
품질 인수다.
