...
class AiJob(Base):
    ...
    __table_args__ = (
        sa.UniqueConstraint(
            "job_type", "resource_type", "resource_id", "input_version", name="uq_ai_job_input"
        ),
    ...
    status: Mapped[str] = mapped_column(sa.String(20), nullable=False, default=JobStatus.PENDING)
    attempts: Mapped[int] = mapped_column(sa.Integer, nullable=False, default=0)
    available_at: Mapped[datetime] = mapped_column(sa.DateTime(), default=utcnow, nullable=False)
...
class IdempotencyKey(Base):
    ...
    response_body: Mapped[dict] = mapped_column(sa.JSON, nullable=False)
    created_at: Mapped[datetime] = mapped_column(sa.DateTime(), default=utcnow, nullable=False)
...
class WorkerHeartbeat(Base):
    ...
    beat_at: Mapped[datetime] = mapped_column(sa.DateTime(), default=utcnow, nullable=False)
