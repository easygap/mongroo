...
class Plant(TimestampMixin, Base):
    ...
    name: Mapped[str] = mapped_column(
        ProtectedText("plants.name"), nullable=False
    )
    ...
    final_form: Mapped[str | None] = mapped_column(
        ProtectedText("plants.final_form"), nullable=True
    )
    emotion_profile: Mapped[dict | None] = mapped_column(
        ProtectedJSON("plants.emotion_profile"), nullable=True
    )
    # 3단계부터 충분한 본문 분석 표본이 모이면 정한다. 새 감정이 조금 추가되는
    # 정도로 외형과 성격이 오락가락하지 않도록 서비스 계층에서 히스테리시스를 둔다.
    growth_branch: Mapped[str | None] = mapped_column(
        ProtectedText("plants.growth_branch"), nullable=True
    )
