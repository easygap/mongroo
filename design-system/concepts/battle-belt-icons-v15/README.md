...
성장결 마크는 지연 로딩이라 **화면에 나온 것만** 받는다. 실기로 본 것은
햇살결 하나뿐이다.

나머지 다섯은 실기 대신 **경로를 밖으로 빼서** 짚는다. 예전에는
`'assets/.../kel/$kel-v1.webp'`가 위젯 안에 문자열로 묻혀 있어서, 실기에서 본
하나 말고는 확인할 방법이 없었다. `expeditionKelIconAsset`으로 빼내고
`expeditionKelsWithIcons` 여섯을 단위 테스트가 전부 돈다 — 화면이 실제로 부르는
그 함수가 여섯 다 번들에 있는 파일을 가리키는지 본다.
