...
성장결 마크가 붙는 자리는 **둘**이다. 벨트 상세의 성장결 태그, 그리고 합동
수호전 머리말의 `잘 통해요`·`잘 안 통해요`. 뒤엣것은 화살표가 통하고 안 통하고만
말하고 **어느 성장결인지는 이름으로만** 있었다 — 앞엣것과 같은 구멍이라 같이
막았다. 서버는 이미 `weak_kel`·`resist_kel` 코드를 보내고 있었는데 앱 모델이
버리고 있었다.

성장결 마크는 지연 로딩이라 **화면에 나온 것만** 받는다. 실기로 본 것은 넷이다.

| 판 | 겹 | 화면에 뜬 마크 |
|---|---|---|
| 돌비늘 장부지기 | 선잠 | `sparkling`(잘 통해요) · `sunny`(잘 안 통해요) |
| 물거울 메아리지기 | 겉꿈 | `rainy`(잘 통해요) · `moonlit`(잘 안 통해요) |

남은 `ember`·`mosaic`은 더 깊은 겹에서만 나온다. 그 둘은 **경로를 밖으로 빼서**
짚는다 — 예전에는 `'assets/.../kel/$kel-v1.webp'`가 위젯 안에 문자열로 묻혀
있어서 실기에서 본 것 말고는 확인할 방법이 없었다. `expeditionKelIconAsset`으로
빼내고 `expeditionKelsWithIcons` 여섯을 단위 테스트가 전부 돈다.
