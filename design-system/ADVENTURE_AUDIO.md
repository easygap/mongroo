...
`normal → story → target`은 세기가 아니라 무게가 다르다. 그래서 세 순간도
서버가 확정한 상태 변화에 붙인다. 연출 타임라인이 아니라 스냅숏이 기준이다.

| cue | 울리는 순간 | 근거 |
|---|---|---|
| `discover-normal` | 새 장소를 찾음 | `memory.discoveries`가 늘어남(발견 노드 도달) |
| `discover-story` | 이야기가 한 걸음 나아감 | `run_thread.current_text`가 바뀜 |
| `discover-target` | 이번 걸음의 목표를 확보 | `run.objective_secured`가 참으로 넘어감 |

- 한 번의 갱신에서 셋이 동시에 열릴 수 있다(목표를 확보하면 이야기도 결말로
  넘어간다). 그때는 **가장 무거운 것 하나만** 낸다.
- **전투로 목표를 확보했으면 아무것도 내지 않는다.** 지역 풀려남 cadence가 이미
  그 순간의 결과음이고, 위에 발견음을 얹으면 무엇이 끝났는지가 오히려 흐려진다.
- 판단은 `expeditionDiscoveryCueFor`에 따로 두고 오디오와 분리했다. 무엇을
  `찾았다`고 볼지는 규칙이라 소리보다 오래 살고, 테스트로 고정할 수 있어야 한다.
...
signature 76종과 발견 cue 3종은 **번들에는 있는데 한 번도 울리지 않았다.** 앱이
tier 대체음 3종만 알고 있었기 때문이다. 만든 것과 들리는 것은 다른 칸이므로
따로 센다.
