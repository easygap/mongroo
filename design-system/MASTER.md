...
[최신 화면·조작 기준](2026-09-22-interface-research.md)이 이전 문서의 색상·글꼴·용어·전투 조작부 규칙보다 우선한다.

- 흰색·차콜의 무채색 조작부와 캐릭터·지역 원화를 사용한다. 베이지·초록, 코발트·오렌지 UI 조합은 사용하지 않는다.
- 일반 화면의 바탕은 `#F7F7F7`, 본문은 `#202020`, 주요 행동은 `#242424`. 전투는 `#1B1B1B` 읽기 면에 밝은 글자를 사용한다.
- 일반 UI는 Wanted Sans. 글자의 길이에 맞춰 공간을 확보하고 핵심 행동명을 말줄임으로 숨기지 않는다.
- 모바일 스킬은 두 칸씩 배치한다. 큰 글씨에서는 스크롤로 모든 행동에 접근할 수 있어야 한다.
- 일기·성장·전투·방어·기력처럼 익숙한 단어로 설명한다. 이전 문서의 ‘성장결·자동 지휘·마음빛·엉킴만 허용’ 같은 용어 강제 규칙은 따르지 않는다.
- 게임의 판정, 저장 데이터, 감정 종류에 따른 공정성은 유지한다. 표현과 실제 결과를 일치시킨다.

## 2026-09-16 적용 기준

현재 탐험 원화와 공간 구성은 [탐험 공간 재설계](2026-09-16-environment-review.md)를 우선한다.
캐릭터 연결과 전투 UI는 [질감 재검수](2026-09-16-clean-art-review.md)를 따른다. 해당 문서의 v2 단순 색면 규칙은 더 이상 원화 기준으로 쓰지 않는다.
지도 진행 구조는 [시각 지도 개편](2026-09-16-visual-exploration.md)을 따른다.
나머지 화면과 플레이 규칙은 [탐험·인터페이스 개편 기록](2026-09-16-field-review.md)을 따른다.
원화·공간 구성에 관해서는 아래 7월 기록보다 이 절을 우선한다. UI 색상·서체는 위 9월 22일 기준으로 대체한다.

- 주요 내비게이션은 오늘 / 기록 / 정원 / 탐험 / 회고. 박물관은 정원과 식물 상세에서 연다.
- 탐험 첫 화면은 지역의 전체 그림 지도다. 실제 스테이지 위치에 터치 표식과 대원을 놓는다. 입구 이미지 아래 설명 카드를 배치하는 구조로 되돌리지 않는다. 순찰과 수첩은 별도 화면에서 열고 닫으면 지도로 돌아온다.
- 출발 버튼은 일기 작성 여부와 관계없이 가능한 탐험을 시작한다. 보상 조건은 별도로 설명한다.
- 기능색은 잉크 `#1C4742`, 선택 연두 `#C4DCA4`, 종이 `#F4F6EF`, 바탕 `#EAF0E8`이다.
  앱 내부의 m 심볼은 잎이 달린 두 색 벡터로 단순화하고, 플랫폼 설치 아이콘은 유지한다. 브랜드 의미를 긴 슬로건으로 반복하지 않는다.
- 본문·행동명·전투 HUD는 Gothic A1. 텍스트 그림자와 도트 글꼴을 기본값으로 쓰지 않는다. 그림만 있는 기술 버튼을 만들지 않는다.
- 기본 패널에는 떠 있는 그림자를 주지 않는다. 실제 행동 결과 외에는 빛·입자·발광을 장식으로 보태지 않는다.
- 사건 원고는 사물, 위치, 변화로 쓴다. 캐릭터 성격은 말의 길이와 행동으로 보여 준다.
- 성공 확률과 실패 비용은 스킬을 적용한 서버 판정과 같아야 한다. 선택의 결과는 구체적인 문장과 실제 획득량으로 보여 준다.
- 일기 감정의 종류가 성장 속도·희귀도·보상의 우열을 만들지 않는 기존 규칙은 유지한다.
