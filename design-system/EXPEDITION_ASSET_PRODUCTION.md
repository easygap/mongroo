...
던전 타일 월드는 2.5D 원화가 아니라 도트를 쓴다. 발밑 타일 아틀라스와 같은
격자·팔레트로 구워야 한 세계로 읽히기 때문이다(`import_expedition_walker.py`).

**지금 있는 것은 공용 한 장뿐이다.** `expedition-walker-v1.png`는 후드를 쓴
여행자 하나이고, 어떤 캐릭터로 들어가도 그 사람이 걷는다. 방향이 읽히는 것을
먼저 잡느라 그렇게 만들었다. 5절 표가 품종마다 무게 중심과 준비 동작을 나누라고
하는데 던전에서는 그 구분이 통째로 없는 셈이다.

앱은 이미 품종 시트를 먼저 찾는다(`expeditionWalkerAssetCandidates`). 아래
규격으로 받아서 구우면 코드를 고치지 않고 그 품종부터 바뀐다.

| 항목 | 값 |
|---|---|
| 파일 | `app/assets/adventure/overworld/expedition-walker-<품종>-v1.png` |
| 품종 코드 | `plant_species.code`의 밑줄을 붙임표로 (`baby_pot` → `baby-pot`) |
| 시트 | 288×480, 3프레임 × 4방향(아래·왼쪽·오른쪽·위) |
| 칸 | 96×120, 열두 칸이 모두 차고 위아래로 잘리지 않을 것 |
| 색 | 불투명 24색, 알파는 0 아니면 255 |
| 검사 | 자글거림 0.16 이하, 튀는 점 0.002 이하 |

방향마다 한 장씩 받는다. 한 장에 열두 칸을 요구하면 칸 사이 일관성이 무너진다.

```
python design-system/scripts/import_expedition_walker.py     --strips 아래.png 왼쪽.png 오른쪽.png 위.png --species cactus
```

품종을 안 주면 공용 시트를 덮는다. 실루엣만으로 방향이 갈려야 한다 — `바라보는
칸의 물건에만 말을 건다`는 규칙이 거기 기대고 있다.
...
| 자산 | 지역당 | 규격 | 제작 방식 |
|---|---:|---|---|
| 통합 탐험 지형 | 1 | 1600×900 WebP + 모바일 960×540 | 랜드마크와 실제 보행로를 그리되 노드·문자는 비움. `import_terrain_art.py`로 들여오고 품질 95를 쓴다(픽셀아트는 압축을 세게 걸면 색 덩어리 경계가 뭉개진다) |
...
**지형 원화는 통행 데이터의 원본이다.** 캐릭터가 이 그림 위를 직접 걸으므로,
그림에 그려진 길이 곧 걸을 수 있는 곳이다. `design-system/scripts/build_walk_masks.py`가
원화를 80×45 격자로 훑어 걸을 수 있는 칸을 뽑아 앱에 글자 지도로 넣는다. 그래서
지형을 다시 뽑으면 **반드시 이 생성기를 다시 돌려야 한다** — 안 돌리면 캐릭터가
없어진 길 위를 걷는다. `--check`가 그림과 데이터의 어긋남을 잡고, `--overlay`가
겹쳐 본 그림을 내준다.

지형을 그릴 때 지켜야 할 것이 둘 있다.

- **길은 배경과 색으로 갈려야 한다.** 밝기만으로는 안 된다(밤 조명이라 수풀도
  길만큼 밝다). 길이 따뜻하거나(모랫길·나무) 확실히 밝고 무채색이면(돌바닥)
  갈린다. 길과 이끼 언덕이 같은 색조면 생성기가 언덕 위도 길로 읽는다.
- **세워진 면을 바닥과 같은 색으로 칠하지 않는다.** 비스듬히 내려다본 그림에서
  벽면은 바닥과 구분이 안 되고, 그러면 캐릭터가 벽을 타고 오른 것처럼 보인다.
  이미 그렇게 된 자리는 지역표에 사각형으로 손수 막았다(별씨앗 보관고의 씨앗
  서랍장·대형 시계). 이 목록이 길어지면 원화를 고치는 편이 낫다.
